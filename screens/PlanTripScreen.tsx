...
const PlanTripScreen = () => {
  ...
  const fetchAIPlaces = async () => {
    ...
    setError(null);
    try {
      // Step 1: Fetch places from OpenAI
      ...
      places.forEach((place) => {
        if (!place.name || !place.description || !place.address) {
          throw new Error("AI response missing required fields (name, description, address)");
        }
      });

      // Step 2: Fetch full details for each place using Google Places API
      const placesWithDetails = await Promise.all(
        places.map(async (place: any) => {
          try {
            // Use findplacefromtext to get place_id
            ...
            // Fetch place details with all fields matching GooglePlacesAutocomplete
  ...
  const handleAddPlaceToItinerary = async (place: any, date: string) => {
    try {
      ...
      console.log("Adding to itinerary:", { place, date, payload });
    ...
    setExpenses((prev) =>
      prev.map((expense) =>
        expense.id === editingExpense.id
          ? { ...expense, ...expenseForm, price: parseFloat(expenseForm.amount) }
          : expense
      )
    );
    setExpenseForm({
  ...
  const getCurrentDayHours = (openingHours: string[]) => {
    ...
    const todayHours = openingHours.find((line) => line.toLowerCase().startsWith(today));
  ...
  const renderPlaceTypes = (types: string[]) => {
    const allowedTypes = ["rv_park", "tourist_attraction", "lodging", "point_of_interest", "establishment"];
    const filteredTypes = types?.filter((type) => allowedTypes.includes(type)) || [];
    const typeColors = {
    ...
    return filteredTypes.map((type, index) => (
      ...
      >
        <Text className={`text-xs font-medium ${typeColors[type] || "text-gray-700"} capitalize`}>
          {type.replace(/_/g, " ")}
        </Text>
      </View>
    ));
  ...
  const renderPlaceCard = (place: any, index: number, isItinerary: boolean = false) => {
    ...
    return (
      <View key={index} className="mb-4 bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
        ...
        >
          <Image
            source={{ uri: place.photos?.[0] || "https://via.placeholder.com/150" }}
            ...
            {place.openingHours?.length > 0 && (
              <View className="mb-4">
                <View className="flex-row items-center">
                  ...
                  <Text className="text-sm font-semibold text-gray-700 ml-1">Today’s Hours</Text>
                </View>
                <Text className="text-sm text-gray-600 mt-1">
            ...
            {place.phoneNumber && (
              <View className="mb-4">
                <View className="flex-row items-center">
                  ...
                  <Text className="text-sm font-semibold text-gray-700 ml-1">Phone</Text>
                </View>
                <Text className="text-sm text-gray-600 mt-1">{place.phoneNumber}</Text>
              </View>
            )}
            ...
            {place.website && (
              <View className="mb-4">
                <View className="flex-row items-center">
                  ...
                  <Text className="text-sm font-semibold text-gray-700 ml-1">Website</Text>
                </View>
                <Text className="text-sm text-blue-600 underline mt-1" numberOfLines={1}>
                  {place.website}
                </Text>
              </View>
            )}
            ...
            {place.reviews?.length > 0 && (
              <View className="mb-4">
                ...
                <Text className="text-sm text-gray-600 italic mt-1">
                  "{place.reviews[0].text.slice(0, 100)}{place.reviews[0].text.length > 100 ? "..." : ""}"
                </Text>
                <View className="flex-row items-center mt-1">
            ...
            {place.types?.length > 0 && (
              <View>
                <View className="flex-row items-center">
                  ...
                  <Text className="text-sm font-semibold text-gray-700 ml-1">Categories</Text>
                </View>
        ...
        >
          <View className="flex-row items-center">
            ...
            <Text className="text-white font-medium ml-2">
              {aiLoading ? "Fetching AI Suggestions..." : "Use AI to Create Itinerary"}
            </Text>
          </View>
        </TouchableOpacity>
          ...
          return (
            <View key={index} className="mb-8">
              ...
              {activities.length > 0 ? (
                ...
                <Text className="text-sm text-gray-500 mb-3">No activities added for this date</Text>
              )}
  ...
  const renderExpenseTab = () => {
    const total = expenses.reduce((sum, expense) => sum + (expense.price || expense.amount || 0), 0);
          ...
          <View key={index} className="mb-4 bg-gray-50 rounded-lg p-3 shadow">
            <View className="flex-row justify-between">
              ...
              <View className="items-end">
                <Text className="text-sm font-semibold">${(expense.price || expense.amount || 0).toFixed(2)}</Text>
    ...
    <SafeAreaView className="flex-1 bg-white">
      <View className="relative w-full h-48">
        ...
        <View className="absolute bottom-[-32px] left-4 right-4 bg-white p-4 rounded-xl shadow-md flex-row justify-between items-center">
          ...
          <View className="items-center">
            <Image
              source={{
                uri: user?.imageUrl || "https://randomuser.me/api/portraits/women/1.jpg",
              }}
      ...
      <View className="flex-row px-4 mt-12 border-b border-gray-200">
        {["Overview", "Itinerary", "Explore", "$"].map((tab, index) => (
          ...
          >
            <Text
              className={`text-base font-medium ${selectedTab === tab ? "text-orange-500" : "text-gray-500"}`}
            >
              {tab}
            </Text>
          </TouchableOpacity>
        ...
        <ScrollView className="px-4 pt-4">
          ...
          <View className="flex-row justify-between mb-6">
            {[
              ...
              <View key={idx} className="w-[48%] bg-white p-4 rounded-lg shadow-sm">
      ...
      {selectedTab === "Explore" && (
        <ScrollView className="px-4 pt-4">
          ...
          <Text className="text-sm text-gray-500">
            Discover more places and activities in {trip.tripName || "this destination"}.
          </Text>
        </ScrollView>
      )}
      ...
      <View className="absolute right-4 bottom-20 space-y-3 items-end">
        ...
        <TouchableOpacity
          onPress={() => navigation.navigate("MapScreen", { places: trip.placesToVisit || [] })}
          ...
          {modalMode === "place" || modalMode === "ai" ? (
            <>
              <Text className="text-lg font-semibold mb-4">
                {selectedDate ? `Add Place to ${dayjs(selectedDate).format("ddd D/M")}` : "Search for a place"}
              </Text>
              {modalMode === "place" && (
                  ...
                  onPress={async (data, details = null) => {
                    try {
                      ...
                      if (json.status !== 'OK' || !json.result) {
                        throw new Error(`Google Places API error: ${json.status || 'No result found'}`);
                      }
                      ...
                      const place = {
                        ...
                        briefDescription:
                          ...
                          `Located in ${d.address_components?.[2]?.long_name || d.formatted_address || "this area"}. A nice place to visit.`,
                        photos: d.photos?.map(
                          (photo: any) => `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${photo.photo_reference}&key=${GOOGLE_API_KEY}`
                        ) || [],
                        formatted_address: d.formatted_address || 'No address available',
                        openingHours: d.opening_hours?.weekday_text || [],
                        phoneNumber: d.formatted_phone_number || '',
                        website: d.website || '',
                        geometry: d.geometry || {
              ...
              <Text className="text-sm font-semibold mb-2 mt-4">Select Date</Text>
              <ScrollView
                horizontal
              ...
              {modalMode === "ai" && aiPlaces.length > 0 && (
                <View className="mt-4">
                  <Text className="text-sm font-semibold mb-2">AI-Suggested Places</Text>
                  <ScrollView style={{ maxHeight: 200 }}>
                    {aiPlaces.map((place, index) => (
                      ...
                      >
                        ...
                        />
                        <View>
                          <Text className="text-sm font-medium">{place.name || "Unknown Place"}</Text>
              ...
              {(trip.placesToVisit || []).length > 0 && (
                <View className="mt-4">
                  <Text className="text-sm font-semibold mb-2">Previously Added Places</Text>
                  <ScrollView style={{ maxHeight: 200 }}>
                    {trip.placesToVisit.map((place: any, index: number) => (
                      <TouchableOpacity
                        key={index}
            ...
            <>
              <Text className="text-lg font-semibold mb-4">
                {modalMode === "editExpense" ? "Edit Expense" : "Add New Expense"}
              </Text>
              <ScrollView>
                ...
                <TextInput
                  ...
                  onChangeText={(text) => setExpenseForm({ ...expenseForm, paidBy: text })}
                ...
                >
                  {splitOptions.map((option, index) => (
                    <TouchableOpacity
                      key={index}
                      onPress={() => setExpenseForm({ ...expenseForm, splitOption: option.value })}
                      className={`px-4 py-2 mr-2 rounded-lg ${
                        expenseForm.splitOption === option.value ? "bg-blue-500" : "bg-gray-100"
                      }`}
                    >
                      <Text
                        className={`text-sm font-medium ${
                          expenseForm.splitOption === option.value ? "text-white" : "text-gray-700"
                        }`}
                      >
                        {option.label}
                      </Text>
                ...
                <TouchableOpacity
                  onPress={modalMode === "editExpense" ? handleEditExpense : handleAddExpense}
                  className="bg-blue-500 p-3 rounded-lg items-center"
                >
